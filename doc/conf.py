# -*- coding: utf-8 -*-
#
# trep documentation build configuration file, created by
...
project = u'trep'
copyright = u'2016, Elliot Johnson'
...
################################################################################
# Version management
class GitError(StandardError): pass
...
def get_version_from_git():
    ...
    if p.returncode != 0:
        raise GitError("git describe failed: '%s'" % stderr)
    return stdout.strip()
...
def get_approx_version():
    ...
    # most recent tag and a '-dev' suffix if appropriate.
    version = get_version()
    if '-' in version:
...
def get_commit_date():
    try:
        p = subprocess.Popen(["git", "log", "-1", "--format=%cd", "--date=short"],
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)
    except EnvironmentError as e:
        raise GitError(e)
    stdout, stderr = p.communicate()
    if p.returncode != 0:
        raise GitError("git log failed: '%s'" % stderr)
    date = map(int,(stdout.strip()).split("-"))
    return (datetime.date(date[0],date[1],date[2])).strftime("%B %d, %Y")
